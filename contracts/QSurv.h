...
public:
  INITIALIZE() {
    _surveyCount = 0;
    // Oracle address should be set via a separate admin procedure
    // or hardcoded after deployment
  }
  ...
  PUBLIC_PROCEDURE_WITH_LOCALS(payout) {
    ...
    if (s.balance < s.rewardPerRespondent) {
      return;
    }
  ...
  PUBLIC_FUNCTION_WITH_LOCALS(getSurvey) {
    output.found = 0;
    output.id = 0;
    output.creator = NULL_ID;
    output.rewardAmount = 0;
    output.rewardPerRespondent = 0;
    output.maxRespondents = 0;
    output.currentRespondents = 0;
    output.balance = 0;
    output.isActive = 0;
